...
(function(window) {'use strict';
	...
	var core = {
		...
		copy : function(object) {
			var l, c, i;
			if (!object || typeof (object) !== 'object' || object instanceof HTMLElement || object instanceof game.Class || (game.Container && object instanceof game.Container)) {
				return object;
		...
		merge : function(original, extended) {
			for (var key in extended) {
				...
				if ( typeof (ext) !== 'object' || ext instanceof HTMLElement || ext instanceof game.Class || ext instanceof game.Container) {
					original[key] = ext;
				} else {
		 ...
		 **/
		fullscreen : function() {
			if (game.system.canvas.requestFullscreen)
				game.system.canvas.requestFullscreen();
			if (game.system.canvas.requestFullScreen)
				game.system.canvas.requestFullScreen();
		},
		...
		require : function() {
			...
			for ( i = 0; i < modules.length; i++) {
				if (modules[i])
					this._current.requires.push(modules[i]);
			}
			return this;
		},
		...
		start : function(scene, width, height, canvasId) {
			if (this._loadQueue.length > 0)
				throw 'Engine modules not ready.';
			...
			if (game.Debug && game.Debug.enabled && !navigator.isCocoonJS)
				this.debug = new game.Debug();
			if (game.DebugDraw && game.DebugDraw.enabled)
				this.debugDraw = new game.DebugDraw();
			if (game.Storage && game.Storage.id)
				this.storage = new game.Storage(game.Storage.id);
			if (game.Analytics && game.Analytics.id)
				this.analytics = new game.Analytics(game.Analytics.id);
		...
		_loadModules : function() {
			...
			if (moduleLoaded && this._loadQueue.length > 0) {
				...
				throw ('Unresolved (circular?) dependencies. ' + 'Most likely there is a name/path mismatch for one of the listed modules:\n' + unresolved.join('\n')
				);
			}
		},
		...
		_boot : function() {
			...
			this.ua = this.device;
			// support for deprecated game.ua
		},
...
game.module('engine.core').require('engine.loader', 'engine.system', 'engine.sound', 'engine.renderer', 'engine.sprite', 'engine.debug', 'engine.storage', 'engine.tween', 'engine.scene', 'engine.pool', 'engine.analytics').body(function() {
});
